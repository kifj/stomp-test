...
public final class Containers implements ArquillianTestContainers {
    ...
    @SuppressWarnings("resource")
    private final GenericContainer<?> etcd = new GenericContainer<>(DockerImageName.parse("quay.io/coreos/etcd:v3.5.28"))
    ...
    private final WildflyContainer wildfly = new WildflyContainer("registry.x1/j7beck/x1-wildfly-stomp-test-it:1.8")
            .dependsOn(database).dependsOn(otel).dependsOn(etcd).withNetwork(network).withEnv("wildfly-testcontainers.properties");
    ...
    public List<GenericContainer<?>> instances() {
        return List.of(etcd, otel, database, wildfly);
    }
