...
public class ShareResourceTest extends AbstractIT {
    ...
    void testAddAndFindShare() {
        ...
        try (var response = client.target(baseUrl).path(PATH_SHARES).request()
            ...
            assertThat(response.getLocation()).isEqualTo(UriBuilder.fromUri(baseUrl).path(PATH_SHARES)
                    .path(PATH_PARAM_KEY).build(share.getKey()));
        }

        Awaitility.waitAtMost(10, TimeUnit.SECONDS).with()
                .pollInterval(Duration.of(500, ChronoUnit.MILLIS)).until(() -> {
                    try {
                        var found = client.target(baseUrl).path(PATH_SHARES).path(PATH_PARAM_KEY)
                                .resolveTemplate(PARAM_KEY, key).request(APPLICATION_JSON).get(Share.class);
                        assertThat(found).isNotNull();
                        assertThat(found.getId()).isNull();
                        assertThat(found.getKey()).isEqualTo(key);
                        assertThat(found.getLinks()).hasSize(3);
                        Assertions.assertThat(found.getLinks()).allSatisfy(link -> {
    ...
    void testGetQuoteNotFound() {
        try (var response = client.target(baseUrl).path(PATH_QUOTES).path(PATH_PARAM_KEY).resolveTemplate(PARAM_KEY, TEST_SHARE)
